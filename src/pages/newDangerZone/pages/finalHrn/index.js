...
class finalHrn extends Component {
  ...
  next = async () => {
    await this.props.setFinalHrn(this.state);
    await this.props.addDangerZone();
    await Actions.push('dangerZones')
  }
  componentDidMount() {
  ...
  render() {
    ...
    const hrn = this.state.finalDamage * this.state.finalOcurrence *
      this.state.finalNumber * this.state.finalFrequence;
    return (
      <ScrollView>
        <Text style={general.subtitle}>HRN final</Text>
        <View style={styles.list2}>
          <View style={styles.listItem}>
            <RNPickerSelect
              ...
              items={damage}
              onValueChange={(value) => this.setState({ finalDamage: value })}
            />
          </View>
          <View style={styles.listItem}>
            <RNPickerSelect
              ...
              items={probability}
              onValueChange={(value) => this.setState({ finalOcurrence: value })}
            />
          </View>
          <View style={styles.listItem}>
            <RNPickerSelect
              ...
              items={number}
              onValueChange={(value) => this.setState({ finalNumber: value })}
            />
          </View>
          <View style={styles.listItem}>
            <RNPickerSelect
              ...
              items={frequence}
              onValueChange={(value) => this.setState({ finalFrequence: value })}
            />
          </View>
        </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(finalHrn)
